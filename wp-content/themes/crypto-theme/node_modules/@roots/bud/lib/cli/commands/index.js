...
export default class BudCommand extends Command {
    ...
    async applyBudArguments(bud) {
         ...
         */
        const override = async (fn) => {
            bud.hasChildren
                ? await Promise.all([bud, ...Object.values(bud.children)].map(async (bud) => await fn(bud)))
                : await fn(bud);
        };
        ...
        isset(args.use) && (await bud.extensions.add(args.use));
    ...
    async applyBudManifestOptions(bud) {
        ...
        bud
            ...
            .when(isset(manifest.paths?.input), bud => bud.hooks.on(`location.@src`, manifest.bud.paths.input))
            .when(isset(manifest.paths?.output), bud => bud.hooks.on(`location.@dist`, manifest.bud.paths.output))
    ...
    async catch(error) {
        ...
        if (this.bud?.dashboard?.instance) {
            ...
            if (this.bud.isProduction) {
                const unmountDashboard = async () => await this.bud.dashboard.instance.waitUntilExit();
            ...
            BudCommand.renderStatic(_jsx(Box, { flexDirection: "column", children: _jsx(Fallback.Error, { error: error }) }));
        }
        // fallthrough
        if (!this.bud || this.bud?.isProduction)
            exit(1);
    }
    /**
     ...
     */
    async makeBud() {
        const applyCliOptionsCallback = async (bud) => {
            await Promise.all([
    ...
    async run(path, userArgs, defaultArgs = []) {
        ...
        if (!(await this.bud.fs.exists(binary))) {
            ...
            if (!pathParts) {
                process.exitCode = 2;
                throw new Error(`Could not find ${signifier} binary\n\nChecked:\n - ${binary}\n - ${checkedPaths
        ...
        this.context.stdout.write(chalk.dim(`${figures.pointerSmall} ${signifier} ${binaryArguments.join(` `)}\n`
        ...
        if (code) {
            this.context.stderr.write(chalk.red(`${figures.cross} exiting with code ${code}\n`));
            return code;
        }
