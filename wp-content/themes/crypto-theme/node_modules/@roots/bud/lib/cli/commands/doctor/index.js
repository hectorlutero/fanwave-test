...
`,
    ...
    async execute() {
        ...
        if (this.mode === `development`) {
            DoctorCommand.renderStatic(_jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { color: "blue", children: "Development server" }), _jsxs(Box, { flexDirection: "row", children: [_jsx(Text, { children: "URL:" }), _jsx(Text, { children: this.bud.hooks
                                    .filter(`dev.url`, new URL(`http://0.0.0.0:3000`))
                                    .toString() })] }), this.bud.hooks
                        .filter(`dev.middleware.enabled`)
                        .includes(`proxy`) ? (_jsxs(Box, { flexDirection: "row", children: [_jsx(Text, { children: "Proxy:" }), _jsx(Text, { children: this.bud.hooks
                                    .filter(`dev.middleware.proxy.options.target`, new URL(`http://0.0.0.0:8000`))
                                    .toString() })] })) : null, _jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { children: "Client scripts:" }), [...this.bud.hooks.filter(`dev.client.scripts`, new Set([]))]
                                .map(fn => fn(this.bud))
                                .map((script, key) => {
                                return (_jsxs(Text, { children: [figures.triangleRightSmall, " ", script] }, key));
                            })] })] }));
        }
        try {
    ...
    formatDepCheck([dependency, requestedVersion], key) {
        if (dependency.startsWith(`@roots`))
            return null;
        const renderMessage = (type) => (_jsxs(Text, { children: [_jsxs(Text, { color: "yellow", children: [figures.warning, `  `, dependency] }), ` `, "is overridden in your project `", type, "`. If you do not require a custom version of", ` `, dependency, " you should remove it."] }, key));
        ...
        return (_jsxs(Text, { children: [_jsxs(Text, { color: "green", children: [figures.tick, " ", dependency] }), ` `, "is managed by bud.js (", requestedVersion, ")"] }, key));
    }
    async ls(path) {
        const formatFilesArray = (files) => {
            return files.map((file, id) => {
                return (_jsxs(Box, { flexDirection: file.children ? `column` : `row`, children: [_jsxs(Text, { children: [_jsx(Text, { dimColor: true, children: file.children ? figures.ellipsis : figures.pointerSmall }), ` `, file.name] }), file.children ? (_jsx(Box, { flexDirection: "column", paddingLeft: 2, children: formatFilesArray(file.children) })) : null] }, `${file.name}-file`));
            });
        };
    ...
    mapEntrypoints(entrypoints) {
        return entrypoints.map(([name, entry]) => {
            return (_jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { children: name }), _jsx(Text, { dimColor: true, children: prettyFormat(entry, {
...
__decorate([
    bind
], DoctorCommand.prototype, "formatDepCheck", null);
