...
export default class Project extends Service {
    ...
    async buildAfter(bud) {
        ...
        await bud.promise(async (bud) => {
            ...
            await bud.fs
                .write(bud.path(`@storage`, bud.label, `debug`, `build.config.yml`), bud.build.config)
                .catch(error => {
    ...
    async compilerDone(bud, stats) {
        ...
        if (!stats)
            return;
        await bud.fs.write(bud.path(`@storage`, bud.label, `debug`, `stats.yml`), {
...
__decorate([
    bind
], Project.prototype, "compilerDone", null);
