    ...
    resolveThemeValue(key, extendedOnly) {
        if (extendedOnly) {
            if (!this.config?.theme?.extend)
                throw new Error(`@roots/bud-tailwindcss: cannot resolve extended theme value when no extended theme config is set.`);
            const value = this.config.theme.extend[key];
            if (!value) {
                throw new Error(`@roots/bud-tailwindcss: extend.${key} is not a valid tailwind theme key.`);
            }
