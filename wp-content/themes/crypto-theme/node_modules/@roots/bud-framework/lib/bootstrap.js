...
const instantiateServices = (app) => async (signifier) => {
    ...
    try {
        ...
        const normalError = error instanceof BudError ? BudError.normalize(error) : error;
        normalError.message = `Error instantiating service ${signifier}: ${normalError.message}`;
        return normalError;
    }
    const label = service.label ?? service.constructor?.name
        ? camelCase(service.constructor.name)
        : signifier;
    app[label] = service;
    logger.log(chalk.blue(`bud.${label}`), figures.arrowLeft, chalk.cyan(!isString(signifier) ? `[object]` : signifier));
 ...
 */
export const bootstrap = async function (bud) {
    bud.fs = new FS(() => bud);
    ...
    bud.hooks
        .fromMap({
        'location.@dist': isString(bud.context.output) && bud.context.output !== ``
            ? bud.context.output
            : `dist`,
        ...
        'location.@src': isString(bud.context.input) && bud.context.input !== ``
            ? bud.context.input
            : `src`,
    ...
    Object.entries(lifecycle).map(([eventHandle, callbackName]) => [...bud.services]
        .map(service => bud[service])
        .filter(Boolean)
        .filter(instance => callbackName in instance)
        .map(instance => {
        bud.hooks.action(eventHandle, instance[callbackName].bind(instance));
    }));
    ...
    await initializeEvents.reduce(async (promised, event) => {
        try {
            await promised;
