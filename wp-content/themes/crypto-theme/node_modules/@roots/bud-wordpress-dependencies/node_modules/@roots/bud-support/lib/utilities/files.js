...
async function getFileInfo(filename) {
    ...
    if (file.type === `module`) {
        file.module = async () => {
            ...
            const outfile = join(paths.storage, `configs`, file.base, `${current.sha1}${file.ext.replace(/(.*)ts$/, `$1js`)}`);
            ...
            const tmpfile = join(paths.basedir, file.dir, `.${file.name}${file.ext.replace(/(.*)ts$/, `$1js`)}`);
