...
export const makeErrorFormatter = (bud) => (errors) => errors
    ...
    if (error.message.includes(unhandledModule)) {
        ...
        const isUnhandledVueModule = error.moduleName?.match(bud.hooks.filter(`pattern.vue`)) &&
        ...
        const isUnhandledSassModule = error.moduleName?.match(bud.hooks.filter(`pattern.sass`)) &&
        ...
        const isUnhandledTsModule = error.moduleName?.match(bud.hooks.filter(`pattern.ts`)) &&
 ...
 */
const filterInternalErrors = (error) => error.message && !error.message?.includes(`HookWebpackError`);
/**
 * Prettify errors
 */
const makePrettifier = (bud) => (error) => {
