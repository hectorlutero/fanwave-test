...
export default class WordPressDependenciesWebpackPlugin {
    ...
    requested;
    constructor(options = { emitWordPressJson: true }) {
        this.options = options;
        this.outputPath = this.options.outputPath ?? `wordpress.json`;
        if (typeof this.options.emitWordPressJson === `undefined` &&
    ...
    apply(compiler) {
        ...
        compiler.hooks.thisCompilation.tap(this.plugin, compilation => {
            ...
            if (this.options.entrypointsPlugin) {
                const hooks = this.options.entrypointsPlugin.getCompilationHooks(compilation);
                hooks.compilation.tap(this.plugin.name, this.extractDependenciesFromCompilation);
                hooks.entrypoints.tap(this.plugin.name, this.tapEntrypointsManifestObject);
            }
            if (this.options?.emitWordPressJson) {
                compilation.hooks.processAssets.tapPromise(this.plugin, async () => {
                    ...
                    compilation.emitAsset(this.outputPath, new compiler.webpack.sources.RawSource(JSON.stringify(this.dependencies, null, 2)));
                });
            }
        });
    }
    extractDependenciesFromCompilation(compilation) {
        for (const { chunks, name } of compilation.entrypoints.values()) {
            if (!name)
                continue;
            for (const chunk of chunks) {
                const records = compilation.chunkGraph.getChunkModules(chunk);
     ...
     */
    processChunkRequest(name, userRequest) {
        if (!name || !userRequest)
            return;
        ...
        for (const request of requested) {
            if (!wordpress.isProvided(request))
                continue;
     ...
     */
    tapEntrypointsManifestObject(entrypoints) {
...
__decorate([
    bind
], WordPressDependenciesWebpackPlugin.prototype, "extractDependenciesFromCompilation", null);
__decorate([
    bind
], WordPressDependenciesWebpackPlugin.prototype, "processChunkRequest", null);
__decorate([
    bind
], WordPressDependenciesWebpackPlugin.prototype, "tapEntrypointsManifestObject", null);
