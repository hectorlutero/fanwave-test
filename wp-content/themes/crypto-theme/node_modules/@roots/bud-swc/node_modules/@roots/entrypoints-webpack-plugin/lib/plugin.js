...
export class EntrypointsWebpackPlugin {
    ...
    addToManifest({ ident, path, type, }) {
        if (path.includes(`.map`) || path.includes(`hot-update`))
            return;
    ...
    apply(compiler) {
        compiler.hooks.thisCompilation.tap(this.constructor.name, compilation => {
            compilation.hooks.processAssets.tapPromise({
                ...
                const hooks = EntrypointsWebpackPlugin.getCompilationHooks(compilation);
                ...
                if (this.options.emitHtml) {
                    new HtmlEmitter(compilation, assets, this.entrypoints, this.options.publicPath).emit();
                }
                let source = {};
                ...
                compilation.emitAsset(this.options.name, new compiler.webpack.sources.RawSource(JSON.stringify(source, null, 2)));
            });
        });
    }
    /**
